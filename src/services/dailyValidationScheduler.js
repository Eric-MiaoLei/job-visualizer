...
export class DailyValidationScheduler {
  constructor({ jobSyncService, autoCrawlService, options = {} }) {
    ...
    this.hour = clampInteger(options.hour, 0, 23, 3);
  ...
  scheduleNextRun() {
    ...
    this.timer = setTimeout(() => {
      void this.runValidation("scheduled");
    }, delayMs);
  }
  ...
  async runValidation(trigger = "manual") {
    ...
    this.state.progress = {
      stage: "starting",
      message: "Preparing daily job validation..."
    };
    ...
    try {
      ...
      this.state.progress = {
        ...
        message: "Daily validation finished, syncing data to storage..."
      };
      ...
      const syncResult = crawlResult.skippedSync
        ...
        : await this.jobSyncService.syncJobs(
          crawlResult.jobs,
          `daily-validation:${crawlResult.outputPath}`,
          {
      ...
      this.state.lastFinishedAt = new Date().toISOString();
      this.state.lastError = error instanceof Error ? error.message : "Unknown daily validation error";
      this.state.progress = null;
      throw error;
    } finally {
